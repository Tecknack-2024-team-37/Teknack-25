    ...
    private void ApplyEngineForce()
{
    ...
    // Ensure Boosting applies speed & acceleration properly
    float currentSpeedLimit = isBoosting ? maxSpeed * boostMultiplier : maxSpeed;
    float currentAcceleration = isBoosting ? accelerationFactor * boostMultiplier : accelerationFactor;

    // Prevent exceeding speed limit
    if (velocityVsUp > currentSpeedLimit) return;
    if (carRigidbody2D.velocity.sqrMagnitude > currentSpeedLimit * currentSpeedLimit) return;
    ...
    // Debugging to confirm boost is applied
    Debug.Log($"Current Speed: {carRigidbody2D.velocity.magnitude} | Is Boosting: {isBoosting} | Speed Limit: {currentSpeedLimit} | Acceleration: {currentAcceleration}");
}
    ...
    public void CollectBoost(float amount)
{
    ...
    Debug.Log("Boost Added! Current Boost: " + currentBoost);

    UpdateBoostUI(); // Ensure UI updates properly
}
  ...
  public void ActivateBoost()
{
    ...
    {
        Debug.Log("Boost NOT Activated - Boost Empty or Already Boosting");
    }
}
...
private IEnumerator BoostRoutine()
{
    ...
    while (elapsed < boostTime && currentBoost > 0)
    {
        ...
        currentBoost -= Time.deltaTime / boostTime * maxBoost; // Decrease boost energy
        UpdateBoostUI();
        yield return null;
    }

    isBoosting = false; // Reset boost effect
}
    ...
    private void OnTriggerEnter2D(Collider2D collider)
{
    ...
    {
        SpeedBoostPickupJump boostPickup = collider.GetComponent<SpeedBoostPickupJump>();
        if (boostPickup != null)
        {
            Debug.Log("Boost Collected!");
            CollectBoost(boostPickup.boostAmount);
            Destroy(collider.gameObject); // Remove boost object after collection
        }
    }
}
